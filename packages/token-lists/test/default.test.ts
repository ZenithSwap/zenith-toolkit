...
import currentZenithswapDefaultList from "../lists/zenithswap-default.json";
import currentZenithswapExtendedtList from "../lists/zenithswap-extended.json";
import currentZenithswapTop15List from "../lists/zenithswap-top-15.json";
import currentZenithswapTop100tList from "../lists/zenithswap-top-100.json";
import { buildList, VersionBump } from "../src/buildList";

const currentLists = {
  "zenithswap-default": currentZenithswapDefaultList,
  "zenithswap-extended": currentZenithswapExtendedtList,
  "zenithswap-top-100": currentZenithswapTop100tList,
  "zenithswap-top-15": currentZenithswapTop15List,
};
...
expect.extend({
  ...
  toBeValidLogo(token) {
    ...
    const refersToLocalLogo =
      token.logoURI === `https://tokens.zenithswap.finance/images/${token.address}.png` ||
      token.logoURI === `https://tokens.zenithswap.finance/images/${token.address.toLowerCase()}.png`;
    if (token.logoURI === "https://tokens.zenithswap.finance/images/0x4e6415a5727ea08aae4580057187923aec331227.png") {
...
describe.each([["zenithswap-default"], ["zenithswap-extended"], ["zenithswap-top-100"], ["zenithswap-top-15"]])(
  "buildList %s",
  (listName) => {
